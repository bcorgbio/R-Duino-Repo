...
```
Unlike that from Figure 8, the relationship between hindwing PC1 and forewing PC2 is not as clear and distinct.

```{r, message= FALSE, echo= FALSE}
summary(lm(hind1~fore2,PC.pic))
```
As expected from Figure 9, the correlation is weak and insignificant at any alpha level. An extremely large amount of variace in the hindwings' PC1 is not explained as indicated by the low R-squared values.
...
### Contributions
Jack: Methods + Intro + bib
John: Intro + Results + bib
Philip: Markdown File + Graphs + Code
Samantha: Discussion
